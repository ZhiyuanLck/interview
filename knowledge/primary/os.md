...
1. 进程和线程的区别
    ...
    - 每个进程在进程表中有对应的表项，里面存储着进程管理相关字段，比如寄存器、程序
      ...
      程表，由于线程共享它所属于的进程的所有资源，线程表表项中只记录程序计数器、寄
      存器、堆栈、线程状态等内容。
...
1. 线程同步
    ...
    4. 记录上锁，可用于进程间共享某个文件的读与写，应用会指定文件中待上锁或解
       锁的字节范围
    5. 信号量，用于提供不同进程间或一个给定进程的不同线程间同步手段的原语，可
       以将每个信号量看成一个由一个整数变量，一个等待进程的链表和两个原子方法
       down、up组成的对象。down方法将信号量减1，如果新值小于0则将正在运行的进
       程叫入等待链表中。up方法将信号量加1，如果新值大于或等于0，则激活链表中
       的一个或多个程序。
    6. 自旋锁，与信号量相似，但是没有等待进程链表，当一个进程发现锁被另外一个
       进程锁着时，它就不停循环直到锁打开。适用与修改数据结构所需要的时间比较
       短的情况，这种情况比信号量更高效，因为将进程插入链表并挂起是比较费时的
       操作。
    7. 屏障，当一个进程到达屏障时会被屏障拦住，直到所有进程都到达该屏障为止
...
1. 函数调用的过程
    发生函数调用时，当前调用函数的状态已经在栈中，被调用函数的状态被压入栈顶，
    函数状态涉及3个寄存器

    - esp 用来存储函数调用栈的栈顶地址，在压栈和退栈时发生变化
    - ebp用来存储当前函数状态的基地址，在函数运行时不变，可以用来索引确定函数
      参数或局部变量的位置
    - eip用来存储即将执行的程序指令的地址，cpu依照eip的存储内容读取指令并执行，
      eip随之指向相邻的下一条指令

    1. 被调用函数的参数按照逆序依次压入栈内，即这些参数会保存在调用函数的函数
       状态内，之后压入栈内的数据都会作为被调用函数的函数状态来保存
    1. 将被调用函数进行调用之后的下一条指令地址作为返回地址压入栈内
    1. 再将当前的ebp寄存器的值（也就是调用函数的基地址）压入栈内，并将ebp寄存
       器的值更新为当前栈顶的地址
    1. 将被调用函数的局部变量等数据压入栈内
    1. 调用结束时，被调用函数的局部变量会从栈内直接弹出，栈顶会指向被调用函数
       的基地址
    1. 将基地址内存储的调用函数的基地址从栈内弹出，并存到ebp寄存器内，调用函数
       的ebp信息得到恢复，此时栈顶指向返回地址
    1. 将返回地址从栈内弹出，并存到eip寄存器内
...
1. 第二次机会页面置换算法
    ...
    如果被修改过则将其写回磁盘。如果R位为1，则清除R位并将其放到队尾。缺点是经
    常要在链表中移动页面。
...
1. 缺页中断的过程
    ...
    1. 如果选择的页面已被修改过，安排该页写回磁盘，并发生一次上下文切换，挂起
       ...
       忙，以免被其他进程占用。
    1. 页面如果是干净的，操作系统查找所需页面在磁盘上的地址，将其装入页框。装
       入时该进程仍然被挂起。
